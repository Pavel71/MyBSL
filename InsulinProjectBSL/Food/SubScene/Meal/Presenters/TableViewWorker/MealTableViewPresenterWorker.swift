  ...
  private static func getDefaultListViewModel(items: [MealRealm]) -> SectionMealViewModel {
    ...
    for mealRealm in items {
      ...
      for product in mealRealm.listProduct {
        ...
        let product = MealViewModel.ProductCell.init(carboIn100Grm: product.carbo, carboInPortion: String(carboInPortionInt), name: product.name, portion: portionString)
