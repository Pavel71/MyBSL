...
class ProductListWorker {
  ...
  static private func getProductListViewModel(product:ProductRealm) -> ProductListViewModel {
    let productListViewModel = ProductListViewModel(insulinValue: product.userSetInsulinOnCarbo, isFavorit: product.isFavorits, carboIn100Grm: product.carboIn100grm, category: product.category, name: product.name, portion: product.portion)
